...
import { IconSearch, IconCopy, IconCheck } from './icons';

const MAX_LINES = 5000;

/**
 * Acompanhamento de logs ao vivo, com busca.
 *
 * O que existia era `docker logs --tail` num terminal: bom pra olhar o passado,
 * inútil pra acompanhar um erro acontecendo. Aqui as linhas chegam por
 * streaming, e a busca filtra sem interromper o fluxo — o que continua
...
export function LiveLogsModal({
  ...
  // Só rola sozinho enquanto o usuário não subiu pra ler algo — puxar a tela de
  // volta pro fim no meio de uma leitura é a forma mais rápida de irritar.
  useEffect(() => {
  ...
  return (
    <Modal title={`Logs — ${title}`} onClose={onClose} maxWidth="max-w-4xl">
      ...
      <div
        ...
        className="h-[55vh] overflow-y-auto rounded-lg bg-slate-950 p-3 font-mono text-[11.5px] leading-relaxed text-slate-300"
      >
        {visible.length === 0 ? (
